...
Eval prints stage progress (`[eval][teacher] Task 3/12: ...`) during model load and per-task scoring. Use `--quiet` to keep stage lines but disable tqdm bars. Under `--parallel`, bars are off on shared stdout by default (stage lines stay prefixed with `[label][gpu=N]`). 3-way parallel needs ~3× VRAM.
